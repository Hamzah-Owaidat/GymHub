...
import { getRoles, exportRoles, deleteRole, type Role } from "@/lib/api/dashboard";
...
export default function RolesPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await getRoles({ page: pagination.page, limit: pagination.limit, search: search || undefined });
      setData(res.data);
      ...
      <div className="flex min-h-0 flex-1 flex-col rounded-2xl border border-stone-200 bg-white dark:border-stone-700 dark:bg-stone-800">
        <div className="flex shrink-0 flex-wrap items-center justify-between gap-4 border-b border-stone-100 p-4 dark:border-stone-700 md:p-6">
          <form onSubmit={handleSearch} className="flex gap-2">
            <input type="text" placeholder="Search roles..." value={search} onChange={(e) => setSearch(e.target.value)} className="rounded-lg border border-stone-200 bg-white px-3 py-2 text-sm dark:border-stone-600 dark:bg-stone-800 dark:text-stone-100" />
              ...
              <tbody>
                {data.map((row, index) => (
                  <tr key={row.id} className="border-b border-stone-100 dark:border-stone-700">
                    ...
                    <td className="px-4 py-3 text-stone-600 dark:text-stone-300">{row.permissions?.length ? row.permissions.map((p) => p.code).join(", ") : "-"}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Link href={"/dashboard/roles/" + row.id} className="text-brand-500 hover:text-brand-600">Edit</Link>
                        <button type="button" onClick={() => handleDelete(row.id, row.name)} className="text-red-500 hover:text-red-600"><TrashBinIcon className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
