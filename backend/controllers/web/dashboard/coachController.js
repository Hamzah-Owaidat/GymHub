...
async function listMySessionUsers(req, res, next) {
  try {
    const coach = await getCurrentCoachForUser(req);
    const [rows] = await pool.query(
      `
        ...
        WHERE u.deleted_at IS NULL
          AND u.is_active = 1
          AND (s.id IS NOT NULL OR us.id IS NOT NULL)
        ORDER BY u.first_name, u.last_name
      `,
      [coach.gym_id, coach.gym_id],
    );
    res.json({ success: true, data: rows });
  } catch (err) {
    next(err);
...
async function createMySession(req, res, next) {
  try {
    const coach = await getCurrentCoachForUser(req);
    const { user_id, session_date, start_time, end_time, price, is_private } = req.body || {};
    ...
    const hasConflict = await Session.hasOverlappingCoachSession(
      coach.id,
      session_date,
      start_time,
      end_time,
    ...
    if (hasConflict) {
      return next(new AppError('You already have a session at this time', 400));
    }
    ...
    const sessionId = await Session.create({
      ...
      price:
        price !== undefined && price !== null && price !== ''
          ? Number(price)
          : Number(coach.price_per_session || 0),
      status: 'booked',
      is_private:
        is_private === true ||
        is_private === 'true' ||
...
async function updateMySession(req, res, next) {
  try {
    const coach = await getCurrentCoachForUser(req);
    const id = Number(req.params.id);
    ...
    if (!existing || Number(existing.coach_id) !== Number(coach.id)) {
      return next(new AppError('Session not found', 404));
    }
    if (existing.status === 'completed') {
      return next(new AppError('Completed sessions cannot be edited', 400));
    }
    ...
    const effectiveUserId = data.user_id !== undefined ? data.user_id : existing.user_id;
    const effectiveDate = data.session_date || existing.session_date;
    const effectiveStart = data.start_time || existing.start_time;
    const effectiveEnd = data.end_time || existing.end_time;

    if (effectiveStart >= effectiveEnd) return next(new AppError('start_time must be before end_time', 400));
    ...
    if (hasConflict) return next(new AppError('You already have a session at this time', 400));
...
async function cancelMySession(req, res, next) {
  try {
    const coach = await getCurrentCoachForUser(req);
    const id = Number(req.params.id);
    if (!id) return next(new AppError('Invalid session id', 400));
    ...
    if (existing.status === 'completed') {
      return next(new AppError('Completed sessions cannot be cancelled', 400));
    }
    if (existing.status === 'cancelled') {
      return res.json({ success: true, session: existing });
    }

    await Session.update(id, { status: 'cancelled' });
    const session = await Session.findById(id);
    res.json({ success: true, session });
  } catch (err) {
    next(err);
...
async function exportMySessions(req, res, next) {
  try {
    const coach = await getCurrentCoachForUser(req);
    const page = 1;
    const limit = 100000;
    ...
    const search = req.query.search || undefined;
    const status = req.query.status || undefined;
    const result = await Session.list({ page, limit, sortBy, sortDir, search, coach_id: coach.id, status });
    const rows = result.data;

    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet('My Sessions');
    sheet.columns = [
      { header: 'ID', key: 'id', width: 10 },
      { header: 'Client', key: 'client', width: 28 },
      { header: 'Client Email', key: 'client_email', width: 30 },
    ...
    rows.forEach((r) => {
      sheet.addRow({
        id: r.id,
        client: `${r.user_first_name || ''} ${r.user_last_name || ''}`.trim(),
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="coach-sessions.xlsx"');
    res.send(Buffer.from(buffer));
  } catch (err) {
    next(err);
  }
